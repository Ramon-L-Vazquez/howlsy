...
function formatResourceType(type: ProjectResourceType) {
  switch (type) {
    case "three_d":
      return "3D";
    case "interactive":
...
export default function GuidedPage() {
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-slate-950 text-white">
        <div className="text-center">
          ...
          <p className="mt-4 text-slate-400">Loading guided project...</p>
        </div>
      </main>
    );
  }
      ...
      <main className="min-h-screen bg-slate-950 text-white">
        <section className="mx-auto flex min-h-screen max-w-3xl flex-col items-center justify-center px-6 py-16 text-center">
          ...
          <p className="mt-5 max-w-2xl text-lg leading-8 text-slate-300">
            Start a project before entering guided mode.
          </p>
  ...
  const progress = (completedCount / project.steps.length) * 100;
  const currentStepComplete = completedStepIds.has(step.id);
      ...
      <main className="min-h-screen bg-slate-950 text-white">
        <section className="mx-auto flex min-h-screen max-w-4xl flex-col items-center justify-center px-6 py-16 text-center">
          ...
          <p className="mt-6 text-sm font-semibold uppercase tracking-[0.2em] text-emerald-400">
            {project.steps.length} of {project.steps.length} steps completed
          </p>
          ...
          <p className="mt-5 max-w-2xl text-lg leading-8 text-slate-300">
            You&apos;ve worked through every guided step for:
          </p>
          ...
          <button
            type="button"
            onClick={handleRestartGuidedMode}
        ...
        >
          <div className="flex flex-wrap items-center justify-between gap-3 text-sm">
            <span className="font-medium text-slate-300">
              {completedCount} of {project.steps.length} steps completed
            </span>
          ...
          <div className="mt-5 flex gap-2 overflow-x-auto pb-1">
            {project.steps.map((projectStep, index) => {
              const isActive = index === currentStep;
              const isFinished = completedStepIds.has(projectStep.id);
              ...
              return (
                ...
                >
                  {isFinished && !isActive ? "✓" : projectStep.order}
                </button>
              );
            })}
          </div>
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <p className="text-xs font-semibold uppercase tracking-wider text-cyan-400">
                              {formatVisualType(visual.resourceType)}
                            </p>
              ...
              {step.troubleshooting.length > 0 && (
                <section className="mt-10">
                  ...
                  <p className="mt-2 text-sm leading-6 text-slate-500">
                    Open the condition that matches what you&apos;re seeing
              ...
              {currentStepComplete && (
                <div className="mt-5 rounded-2xl border border-emerald-900 bg-emerald-950/30 p-4">
                  <p className="flex items-center gap-2 font-semibold text-emerald-300">
                    ...
                    This step has been marked complete.
                  </p>
                </div>
              )}
            </div>
